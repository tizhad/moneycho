...
const COPY = {
  en: {
    ...
    principles: [
      ...
      },
      {
        ...
        body: 'Our calculators contain no paid placement, and a commission never changes a number. Some outbound links may earn us a commission — we label them, and we explain our full revenue model openly.',
      },
      {
        n: '03',
        title: 'Always free',
  ...
  nl: {
    ...
    principles: [
      {
        n: '01',
        title: 'Laat de berekening zien',
        body: 'Elke calculator toont de formule achter het resultaat. Geen zwarte dozen. Je kunt elk getal dat we je geven zelf verifiëren.',
      },
      {
        n: '02',
        title: 'Transparant over geld',
        body: 'Onze calculators bevatten geen betaalde plaatsing, en een commissie verandert nooit een getal. Sommige uitgaande links kunnen ons commissie opleveren — die labelen we, en ons volledige verdienmodel leggen we open uit.',
      },
      {
        n: '03',
        title: 'Altijd gratis',
        body: 'Onze calculators en gidsen zijn gratis, voor altijd. Financiële kennis mag geen prijskaartje hebben.',
      },
      {
        n: '04',
        title: 'Regelmatig bijgewerkt',
        body: 'Belastingschijven, rentetarieven en wettelijke cijfers veranderen. We werken onze modellen bij wanneer dat nodig is en vermelden de bron.',
      },
    ],
    ctaHeading: 'Vragen of correcties?',
    ctaBody:
      'Een fout gevonden in een berekening of een belastingcijfer dat bijgewerkt moet worden? Laat het ons weten.',
...
export default async function AboutPage({
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-15 md:py-32">
      {/* Hero */}
      ...
      <div className="grid md:grid-cols-[1fr_2px_1fr] gap-0 mb-24">
        <div className="space-y-12 md:pr-16">
          {c.sections.slice(0, 3).map((s) => (
            <div key={s.heading}>
              <h2 className="font-serif font-bold text-lg text-emerald-deep mb-3">
                {s.heading}
              </h2>
              <p className="text-[0.95rem] leading-[1.8] text-emerald-deep/65">
                {s.body}
              </p>
            </div>
          ))}
        ...
        <div className="grid sm:grid-cols-2 gap-px bg-emerald-deep/10 border border-emerald-deep/10">
          {c.principles.map((p) => (
            <div key={p.n} className="bg-paper p-10">
              <span className="font-serif text-3xl text-gold block mb-4">
                {p.n}
              </span>
              <h3 className="font-serif text-xl text-emerald-deep mb-3">
                {p.title}
              </h3>
      ...
      <div className="bg-cream-deep border border-emerald-deep/10 p-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
        <div>
          <p className="font-serif text-2xl text-emerald-deep mb-2">
            {c.ctaHeading}
          </p>
