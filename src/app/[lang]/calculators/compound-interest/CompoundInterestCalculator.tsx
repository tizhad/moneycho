...
const FREQS = {
  ...
  nl: [
    { label: "Maandelijks", value: 12 },
    { label: "Kwartaal", value: 4 },
    { label: "Jaarlijks", value: 1 },
  ],
} as const;
...
const T = {
  en: {
    ...
    desc: "See how your money grows over time. The longer your timeline, the more compounding works in your favour.",
    ...
    currency: "Currency",
    currencyHint: "Auto-detected · change if incorrect",
    futureValue: "Future Value",
    afterYears: (y: number | "") => `after ${y} years`,
  ...
  nl: {
    ...
    h1: "Rente op Rente Calculator",
    desc: "Zie hoe je geld groeit in de loop van de tijd. Hoe langer je belegt, hoe harder de samengestelde rente voor je werkt.",
    startingAmount: "Startbedrag",
    monthlyContribution: "Maandelijkse inleg",
    annualRate: "Jaarlijkse rente",
    timePeriod: "Looptijd",
    timeSuffix: "jr",
    compoundFreq: "Aanrente-frequentie",
    currency: "Valuta",
    currencyHint: "Automatisch gedetecteerd · wijzig indien nodig",
    futureValue: "Eindwaarde",
    afterYears: (y: number | "") => `na ${y} jaar`,
    totalContributed: "Totaal ingelegd",
    interestEarned: "Rente verdiend",
    breakdown: "Verdeling",
    breakdownTooltip:
      "Deze balk laat zien waar je eindwaarde vandaan komt. Het groene gedeelte is geld dat je zelf hebt ingelegd; het gouden gedeelte is rente verdiend door aanrentering. Hoe langer je belegt, hoe groter het gouden gedeelte.",
    contributions: "Inleg",
    growthOverTime: "Groei in de tijd",
    yearLabel: "Jaar",
...
export default function CompoundInterestCalculator({ lang }: { lang?: string }) {
  ...
  const chartData = useMemo(() => {
    ...
    return Array.from({ length: len + 1 }, (_, yr) => {
      ...
      const balance =
        rMonthly === 0
          ? p + m * n
          : p * Math.pow(1 + rMonthly, n) +
            (m * (Math.pow(1 + rMonthly, n) - 1)) / rMonthly;
      ...
      <div className="grid lg:grid-cols-2 gap-16 items-start">
        ...
        <div>
          ...
          <div className="space-y-6">
            ...
            <Field label={t.monthlyContribution}>
              <NumberInput
                value={monthly}
                onChange={setMonthly}
                prefix={selected?.symbol ?? "$"}
            ...
            <Field label={t.annualRate}>
              <NumberInput
                value={rate}
                onChange={(v) => setRate(v === "" ? "" : Math.min(100, v as number))}
                suffix="%"
              />
            </Field>
            <Field label={t.timePeriod}>
              <NumberInput value={years} onChange={setYears} suffix={t.timeSuffix} />
            </Field>
            <Field label={t.compoundFreq}>
            ...
            <div className="relative" ref={switcherRef}>
              ...
              >
                ...
                <span className="text-emerald-deep/40 text-sm">
                  {showSwitcher ? "▲" : "▼"}
                </span>
              </button>
              {showSwitcher && (
              ...
              <p className="text-xs text-emerald-deep/30 mt-2">{t.currencyHint}</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-emerald-deep text-paper p-8">
            ...
            <p className="text-paper/50 text-sm mt-2">{t.afterYears(years)}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-paper border border-emerald-deep/10 p-6">
              ...
              <p
                className="font-display text-2xl font-bold text-emerald-deep"
          ...
          <div className="bg-paper border border-emerald-deep/10 p-6">
            ...
            <div className="flex justify-between text-xs text-emerald-deep/50">
              <span>
                ● {t.contributions}{" "}
                {Math.round((result.totalContributions / result.futureValue) * 100)}%
              </span>
              <span>
                ● {t.interestEarned}{" "}
                {Math.round((result.totalInterest / result.futureValue) * 100)}%
              </span>
            </div>
          </div>
        </div>
            ...
            <AreaChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              ...
              <Tooltip
                ...
                content={({ active, payload, label }) => {
                  ...
                  const contrib =
                    (payload.find((p) => p.dataKey === "contributions")?.value as number) ?? 0;
                  const interest =
                    (payload.find((p) => p.dataKey === "interest")?.value as number) ?? 0;
                  return (
                    <div className="bg-emerald-deep text-paper text-xs p-3 rounded shadow-lg">
                      ...
                      <p className="text-paper/70">
                        {t.total}: {fmt(contrib + interest)}
                      </p>
                      <p className="text-paper/70">
                        {t.contributions}: {fmt(contrib)}
                      </p>
                      <p className="text-gold font-semibold">
                        {t.interestEarned}: {fmt(interest)}
                      </p>
                    </div>
                  );
                }}
          ...
          <div className="flex gap-6 mt-3">
            <div className="flex items-center gap-2">
              <div
                ...
                style={{ background: `${CHART_COLORS.emerald}55` }}
              />
              <span className="text-xs text-emerald-deep/50">{t.contributions}</span>
            </div>
            <div className="flex items-center gap-2">
              <div
                className="w-4 h-2 rounded-sm"
                style={{ background: `${CHART_COLORS.gold}88` }}
              />
              <span className="text-xs text-emerald-deep/50">{t.interestEarned}</span>
            </div>
          </div>
        </div>
      )}
